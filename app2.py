...
def run_script(script_name):
    ...
    def check_and_run_comparison():
        all_completed = all(
            status.startswith('Completed') for script, status in script_status.items() if script != 'Run_Comparison.py')
        if all_completed:
            logging.debug("All scripts completed. Starting Run_Comparison.py.")
            run_script('Run_Comparison.py')
...
# if __name__ == '__main__':
#     app.run(host='pete.flinnsci.com', port=5000, debug=True)
...
if __name__ == '__main__':
    ssl_context = None
    try:
        pfx_path = r'D:\svc_webscrape\Webscrapping- Webpage V2.2 (WebSocket)\SSL Certificate\pete_flinnsci_com.pfx'
    ...
    app.run(host='pete.flinnsci.com', port=5000, debug=True, ssl_context=ssl_context)